      ...
      let tensor = match tensor.datum_type() {
        DatumType::F32 => {
          ...
          Ok(tensor.map(|x| {
            ...
            // Preserve positive stabilizers such as LayerNorm epsilon when fixed-point
            // rounding would otherwise turn them into zero.
...
pub fn load_file(filename: &str) -> (Graph, Vec<(ArrayD<Fr>, DatumType)>) {
  ...
  graph.outputs = onnx_graph
    .output
    .iter()
    .map(|output| {
      let locations =
        outputs_idx.get(&output.name).unwrap_or_else(|| panic!("ONNX graph output {} is not produced by the compiled graph", output.name));
      assert_eq!(locations.len(), 1, "ONNX graph output {} has an ambiguous producer", output.name);
      let location = locations[0];
      assert!(location.0 >= 0, "an ONNX graph input cannot be used directly as a graph output");
      location
    })
    .collect();
  assert!(!graph.outputs.is_empty(), "ONNX graph must declare at least one output");
...
mod pocomp_tests {
  ...
  #[test]
  fn compiled_graph_tracks_declared_onnx_outputs() {
    let root = env!("CARGO_MANIFEST_DIR");
    std::env::set_var("ZKTORCH_CONFIG", format!("{root}/config.yaml"));
    let sample = format!("{root}/sample.onnx");
    let (graph, _) = load_file(&sample);
    assert_eq!(graph.outputs.len(), 1);
    assert!(graph.outputs[0].0 >= 0);
  }
}
