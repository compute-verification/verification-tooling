...
struct CQLinSetupCommon {
  L_H_i_x: Vec<G1Projective>,
  L_V_i_x_n: Vec<G1Projective>,
  L_V_i_x: Vec<G1Projective>,
  U: Vec<Vec<G1Projective>>,
  U_P_R: Vec<Vec<G1Projective>>,
  U2: Vec<Vec<G2Projective>>,
  V: Vec<G1Projective>,
  srs_star: Vec<Vec<G1Projective>>,
}
...
fn cqlin_setup_common(srs: &SRS, m: usize, n: usize) -> Arc<CQLinSetupCommon> {
  let key = cqlin_setup_cache_key(srs, m, n);
  if let Some(common) = CQLIN_SETUP_CACHE.lock().expect("CQLin setup cache lock poisoned").get(&key) {
    return Arc::clone(common);
  }
  ...
  let mut L_V_i_x: Vec<G1Projective> = srs.X1P[..m].par_iter().map(|x| (*x).into()).collect();
  util::ifft_in_place(domain_m, &mut L_V_i_x);
  ...
  let common = Arc::new(CQLinSetupCommon {
    L_H_i_x,
    L_V_i_x_n,
    L_V_i_x,
    U,
  ...
  let mut cache = CQLIN_SETUP_CACHE.lock().expect("CQLin setup cache lock poisoned");
  Arc::clone(cache.entry(key).or_insert_with(|| Arc::clone(&common)))
}
...
impl BasicBlock for CQLinBasicBlock {
  ...
  fn setup(&self, srs: &SRS, model: &ArrayD<Data>) -> (Vec<G1Projective>, Vec<G2Projective>, Vec<DensePolynomial<Fr>>) {
    ...
    let domain_m = GeneralEvaluationDomain::<Fr>::new(m).unwrap();
    let domain_2m = GeneralEvaluationDomain::<Fr>::new(2 * m).unwrap();
    let common = cqlin_setup_common(srs, m, n);

    let S: Vec<Vec<_>> = (0..m)
      .into_par_iter()
      .map(|i| (0..n).map(|j| (common.U[i][j] * domain_m.element(i).inverse().unwrap() - common.V[j]) * model[i].raw[j]).collect())
      .collect();
    ...
    setup.extend_from_slice(&common.L_V_i_x_n);
    setup.extend_from_slice(&common.L_V_i_x);
    setup.extend_from_slice(&common.L_H_i_x);
