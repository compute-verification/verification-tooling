...
struct InputData {
  #[serde(default)]
  input_data: Option<Vec<Vec<f64>>>,
  #[serde(default)]
  input_data_quantized: Option<Vec<Vec<i64>>>,
}
...
pub fn load_inputs_from_json_for_onnx(onnx_name: &str, json_name: &str) -> Vec<ArrayD<Fr>> {
  ...
  assert!(
    json.input_data.is_some() ^ json.input_data_quantized.is_some(),
    "input JSON must contain exactly one of input_data or input_data_quantized"
  );
  ...
  for (i, onnx_input) in onnx_graph.input.iter().enumerate() {
    ...
    let input = if let Some(quantized) = &json.input_data_quantized {
      quantized[i].iter().copied().map(Fr::from).collect()
    } else {
      let input_data = json.input_data.as_ref().unwrap();
      match t.elem_type() {
        DataType::Float | DataType::Float16 | DataType::Double => input_data[i]
          .iter()
          .map(|x| {
          ...
          .collect(),
        DataType::Int8 | DataType::Int16 | DataType::Int32 | DataType::Int64 => input_data[i].iter().map(|x| Fr::from(*x as i32)).collect(),
        DataType::Uint8 | DataType::Uint16 | DataType::Uint32 | DataType::Uint64 => input_data[i].iter().map(|x| Fr::from(*x as u32)).collect(),
        DataType::Bool => input_data[i].iter().map(|x| Fr::from(*x as u8)).collect(),
