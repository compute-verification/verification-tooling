...
impl Layer for ReduceMeanLayer {
  fn graph(
    ...
    let mut mean_shape = input_shapes[0].clone();
    mean_shape[input_shapes[0].len() - 1] = 1;
    let div_output = super::add_rounded_constant_division(
      &mut graph,
      (sum_output, 0),
      input_shapes[0][input_shapes[0].len() - 1].try_into().expect("ReduceMean divisor does not fit u32"),
      &mean_shape,
    );
    ...
    if axes.len() == 2 {
      ...
      let mut second_mean_shape = mean_shape.clone();
      second_mean_shape[input_shapes[0].len() - 2] = 1;
      let div_output1 = super::add_rounded_constant_division(
        &mut graph,
        (sum_output1, 0),
        input_shapes[0][input_shapes[0].len() - 2].try_into().expect("ReduceMean divisor does not fit u32"),
        &second_mean_shape,
      );
      graph.outputs.push((div_output1, 0));
      (graph, vec![second_mean_shape], vec![input_types[0]])
    } else if axes.len() == 1 {
      graph.outputs.push((div_output, 0));
      (graph, vec![mean_shape], vec![input_types[0]])
    } else {
